...
- Switch to root user.
- Clone the repo.
- Go inside repo directory then proceed the below steps
```
pip install aws-tagger
```

-If getting any error while running above command then first update the setup tools and proceed for aws-tagger installation
```
...
```
aws-tagger doesn't support network load balancer 
```
